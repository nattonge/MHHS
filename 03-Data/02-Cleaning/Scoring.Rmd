...
```
# Metadata and attention checks

AC1 - We care about the quality of our survey data. For us to get the most accurate measures of your opinions, it is important that you provide thoughtful answers to each question in this survey. Do you commit to providing thoughtful answers to the questions in this survey? (1=yes)

*Notes that AC2 is wonky in Qualtrics. Response options are coded:* 
1 = strongly disagree
2 = Disagree
...
*Consider whether you want to know if participants closely read the question and followed instructions, or where they perhaps answering honestly*

AC3 - AC3 - Based on the text you read above, what color have you been instructed to enter? (correct answer is 3-green)

AC-4 - We care about the quality of our survey data. Knowing you will receive credit for completing this survey regardless of your answer, please answer the following question: Should we use your data? (1=yes)


```{r metadata}
# saving the start, end, progress, duration etc.
meta_data<-mhhs_data %>% select(StartDate:ResponseId, contains("Consent"),starts_with("AC")) %>% mutate(Duration_in_seconds=Duration..in.seconds.)
...
#finding 3 of 4 attention checks and a reasonable duration
meta_data %>% filter(AC1==1&AC3==3&AC4==1&AC2==6|AC1==1&AC3==3&AC4==1|AC1==1&AC3==3&AC2==6|AC1==1&AC4==1&AC2==6|AC3==3&AC4==1&AC2==6|AC1==1&AC3==3&AC4==1&AC2==6|AC1==1&AC3==3&AC4==1&AC2==2|AC1==1&AC3==3&AC2==2) %>%  filter(Duration_in_seconds>1200&Duration_in_seconds<58519)

# Determine outliers with median value taken for a new filter
median(boxplot(meta_data$Duration_in_seconds)$out) #median outlier value
meta_data_medianfilter<-meta_data %>% filter(Duration_in_seconds<58519) 
(boxplot(meta_data_medianfilter$Duration_in_seconds)$stats)
mean(meta_data_medianfilter$Duration_in_seconds) #a more reasonable mean. SD is too high to use it to find a lower bound so will just use intuition. 

#save(meta_data,file="../04-ProcessedData/meta_data12202023.RDS")
...
#save(mhhs_demotbl,file="../04-ProcessedData/Demotable122023.RDS")
```
...
#save(ghsql,file="../04-ProcessedData/ghsql122023.RDS")
```
...
dass_scatter<-dass %>%  select(Dass21_Stress,Dass21_Anxiety, Dass21_Depression) %>% 
...
#save(dass,dass_scatter,file="../04-ProcessedData/dass122023.RDS")
```
# ISMI29
...
#save(ismi,file="../04-ProcessedData/ismi122023.RDS")
```
...
# checking the correlations between DDI and other measures ensure they make sense
...
#save(bfne,file="../04-ProcessedData/bfne122023.RDS")
...
#save(trs,file="../04-ProcessedData/trs122023.RDS")
...
#save(tps,file="../04-ProcessedData/tps122023.RDS")
...
# selecting numeric responses only
...
#save(eis,eis_resp,eis_id,file="../04-ProcessedData/eis122023.RDS")
...
```{r}
#BII scoring - straightforward except for this wonky numbering business
...
#save(bicii,file="../04-ProcessedData/bicii122023.RDS")
...
```{r Kayleigh honors project}
kayleighdata<-cbind(mhhs_data$ResponseId,meta_data,mhhs_demotbl,bicii,eis,ghsql,ismi,trs,tps)
#write.csv(kayleighdata,"../06-Analyses/03-Kayleigh/kayleighdata21224.csv")
```
