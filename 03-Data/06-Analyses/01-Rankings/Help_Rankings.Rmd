...
```{r plct mhhs}
personal<-mhhs_data %>% select(response_id,ghsq_rank_pers_1:ghsq_rank_pers_9)
personal_demo<-mhhs_data %>% select(response_id,ghsq_rank_pers_1:ghsq_rank_pers_9,race_ethn2,bic_1,gender,sexor,lang_1)
personal<-personal[complete.cases(personal),]
personal_demo<-left_join(personal,personal_demo)
personal <- personal %>% select(-response_id)
personal.rank<-as.rankings(personal) 

personal.g<-group(personal.rank,index=rep(seq_len(nrow(personal)),1))

tree_gendersex<-pltree(personal.g~.,data=personal_demo[c("gender","sexor")])
plot(tree_gendersex)

tree_raceethn<-pltree(personal.g~.,data=personal_demo[c("race_ethn2","bic_1","lang_1")])
plot(tree_raceethn)

## Sui
suic<-mhhs_data %>% select(response_id,ghsqo_rank_sui_1:ghsqo_rank_sui_9)
suic_demo<-mhhs_data %>% select(response_id,ghsqo_rank_sui_1:ghsqo_rank_sui_9,race_ethn2,bic_1,gender,sexor,lang_1)
suic<-suic[complete.cases(suic),]
suic_demo<-left_join(suic,suic_demo)
suic <- suic %>% select(-response_id)
suic.rank<-as.rankings(suic) 

suic.g<-group(suic.rank,index=rep(seq_len(nrow(suic)),1))

tree_gendersex<-pltree(suic.g~.,data=suic_demo[c("gender","sexor")])
plot(tree_gendersex)

tree_raceethn<-pltree(suic.g~.,data=suic_demo[c("race_ethn2","bic_1","lang_1")])
plot(tree_raceethn)

tree_multimar<-pltree(suic.g~BIC_1*SEXOR,data=suic_demo, maxdepth=3,minsize=0.05*nrow(suic))
plot(tree_raceethn)

#creating a multiply marginalized field
suic_demo<-suic_demo %>% mutate(multimar=ifelse(bic_1>0 & sexor>1,1,0))
personal_demo<-personal_demo %>% mutate(multimar=ifelse(bic_1>0 & sexor>1,1,0))

tree_multimar<-pltree(suic.g~.,data=suic_demo[c("race_ethn2","sexor","multimar")])
plot(tree_multimar)
```
...
# PLM models
pers.mod_mle <- PlackettLuce(personal.rank, npseudo=0) #for personal, everything worse than intimate partners
suic.mod_mle <- PlackettLuce(suic.rank, npseudo=0 ) #for suic, mental health prof were marginally lower


summary(pers.mod_mle,ref="GHSQ_RankPers_5") #for personal problem, friends, rps, and parents all preferred
summary(suic.mod_mle,ref="GHSQO_RankSUI_5") 


personal_demo$racebinary<-ifelse(personal_demo$race_ethn2>1,1,0) #binary for race/ethnicity
ranks_race<-summary(rol(personal.rank,personal_demo$racebinary)) #faculty rankings 2 is the groupd rankings class of object. allows the rankings to be linked to covariates with group-specific values

#NAT - adding the p-values and z-scores
ranks_race@coef %>% as.data.frame(.) %>% mutate(z_score=Estimate/`Std. Error`) %>% rowwise() %>% mutate(p_value=2*(pnorm(abs(z_score),lower.tail = FALSE))) %>% ungroup() %>% slice_tail(n=9)
